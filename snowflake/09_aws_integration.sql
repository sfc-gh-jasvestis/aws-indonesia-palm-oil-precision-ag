-- ============================================================================
-- 09_AWS_INTEGRATION.SQL — AWS services for Precision Agriculture Analytics
...
CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION aws_indonesia_palm_oil_precision_ag_BEDROCK_EAI
  ALLOWED_NETWORK_RULES = (PALM_OIL_PRECISION_AG.APP.BEDROCK_NETWORK_RULE)
  ALLOWED_AUTHENTICATION_SECRETS = (PALM_OIL_PRECISION_AG.APP.AWS_BEDROCK_SECRET)
  ENABLED = TRUE
  COMMENT = 'Bedrock access for Precision Agriculture Analytics';
...
CREATE OR REPLACE FUNCTION APP.BEDROCK_GENERATE(prompt VARCHAR)
  ...
  EXTERNAL_ACCESS_INTEGRATIONS = (aws_indonesia_palm_oil_precision_ag_BEDROCK_EAI)
  SECRETS = ('aws_creds' = PALM_OIL_PRECISION_AG.APP.AWS_BEDROCK_SECRET)
AS $$
...
-- Stream ARN: arn:aws:kinesis:ap-southeast-1:<YOUR_AWS_ACCOUNT_ID>:stream/aws-indonesia-palm-oil-precision-ag-stream

CREATE OR REPLACE PIPE RAW.REALTIME_PIPE
  AUTO_INGEST = TRUE
  INTEGRATION = 'aws_indonesia_palm_oil_precision_ag_S3_INT'
  COMMENT = 'Auto-ingest from Kinesis via S3 delivery stream'
AS
COPY INTO RAW.BLOCKS
